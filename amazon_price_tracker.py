...
def trackPrice():
    ...
    if price > WANTED_PRICE:
        ...
        getPrice()
        enviarEmail()
        print(f'Eba!!!! Chegou a hora de comprar! O produto atingiu um preço abaixo do seu preço alvo: {WANTED_PRICE}')
...
def getPrice():
    ...
    print(f'Título do item que você busca: {title}')
    print(f'Preço do item: {price}')

    return price

# função que envia e-mail automático caso tenha o preço for abaixo do preço alvo 
def enviarEmail():
    # criando conexão com o servidor do GMAIL
    server = smtplib.SMTP('smtp.gmail.com', 587)
    
    # identificando-se para o servidor como uma troca de e-mails
    server.ehlo()
    
    # encriptando a conexão
    server.starttls()
    
    # se identificando novamente como uma troca de e-mails
    server.ehlo()
    ...
    '''
    Definindo o conteúdo da mensagem: Assunto, Corpo do E-mail e Mensagem
    '''
    assunto = '"Baixou" '
    corpo = 'https://www.amazon.com.br/Teclado-Mecanico-K7-Rainbow-Fortrek-2019-windows/dp/B07NRTQXR6/ref=sr_1_2?__mk_pt_BR=%C3%85M%C3%85%C5%BD%C3%95%C3%91&crid=3ASRDF755788U&dchild=1&keywords=teclado+mecanico+redragon&qid=1594258247&sprefix=teclado+meca%2Caps%2C282&sr=8-2 '
    msg = f'Subject: {assunto}\n\n{corpo}'

    # enviando o e-mail, definindo (REMETENTE, DESTINATARIO,MENSAGEM) 
    server.sendmail(email, email, msg)
    
    # notificando que o envio foi realizado com sucesso
    print('E-mail enviado! :)')
    
    # fechando a conexão do e-mail
    quit()
